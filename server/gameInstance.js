// --- START OF FILE server/gameInstance.js (FINÁLNÍ ROBUSTNÍ VERZE) ---
...
class GameInstance {
    constructor(gameCode, players) {
        ...
        this.buildings = {};
        this.visibilityMaps = {};
        this.dirtyData = {}; // Která data se změnila v tomto ticku
    }

    initializeGame() {
        this.board = this.generateMap(GAME_CONFIG.GRID_SIZE);
        const startPositions = [ /* ... startovní pozice ... */ ];
        Object.values(this.players).forEach((player, index) => {
            // ... logika pro nastavení hráčů, surovin, startovních jednotek ...
        });

        // Vrací počáteční balíček dat, který LobbyManager pošle hráčům
        return {
            gameCode: this.code,
            config: GAME_CONFIG,
    ...
    // gameTick nyní NEPOSÍLÁ data. Jen je spočítá a vrátí.
    gameTick(deltaTime) {
        ...
        // Vrací data pro odeslání, aby se o to postaral LobbyManager
        return this.createUpdatePackets();
    }
    
    // Zde je VEŠKERÁ herní logika (updateResource, updateCombat, atd.)
    // Je identická s předchozí verzí, takže ji pro přehlednost zkracuji.
    // DŮLEŽITÉ: Ve vašem souboru musí být VŠECHNY tyto metody z předchozí verze!
    // Sem je pro zkrácení odpovědi nevkládám, ale jejich kód je správný.
    
    updateResourceProduction(deltaTime) { /* ... kód z minulé verze ... */ }
    updateBuildingConstruction(deltaTime) { /* ... kód z minulé verze ... */ }
    updateUnitTraining(deltaTime) { /* ... kód z minulé verze ... */ }
    updateUnitMovement(deltaTime) { /* ... kód z minulé verze ... */ }
    updateCombat(deltaTime) { /* ... kód z minulé verze ... */ }
    updateVisibility() { /* ... kód z minulé verze ... */ }
    handlePlayerAction(socketId, action) { /* ... kód z minulé verze ... */ }
    createUnit(player, unitType, pos) { /* ... kód z minulé verze ... */ }
    createBuilding(player, structureType, x, y) { /* ... kód z minulé verze ... */ }
    findClosestEnemy(attacker, allUnits, allBuildings, deadEntities) { /* ... kód z minulé verze ... */ }
    generateMap(size) { /* ... kód z minulé verze ... */ }
    ...
    // Vytvoří balíčky dat pro VŠECHNY hráče
    createUpdatePackets() {
        const allPackets = {};
...
// Zde je kompletní kód pro všechny zkrácené metody. Zkopírujte je do svého souboru.
GameInstance.prototype.updateResourceProduction = function(deltaTime) {
...
GameInstance.prototype.updateBuildingConstruction = function(deltaTime) {
    Object.values(this.buildings).forEach(b => {
...
GameInstance.prototype.updateUnitTraining = function(deltaTime) {
    Object.values(this.buildings).forEach(b => {
...
GameInstance.prototype.updateUnitMovement = function(deltaTime) {
    Object.values(this.units).forEach(u => {
...
GameInstance.prototype.updateCombat = function(deltaTime) {
...
GameInstance.prototype.updateVisibility = function() {
...
GameInstance.prototype.handlePlayerAction = function(socketId, action) {
...
GameInstance.prototype.createUnit = function(player, unitType, pos) {
...
GameInstance.prototype.createBuilding = function(player, structureType, x, y) {
    const config = GAME_CONFIG.BUILDINGS[structureType];
...
GameInstance.prototype.findClosestEnemy = function(attacker, allUnits, allBuildings, deadEntities) {
...
GameInstance.prototype.generateMap = function(size) {
