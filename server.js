...
function updateVisibility(game) {
    game.players.forEach(player => {
        ...
        Object.values(game.buildings).filter(b => b.ownerId === player.id && b.buildProgress === 1).forEach(b => {
...
function updateUnitMovement(game, deltaTime) {
    Object.values(game.units).forEach(u => {
        ...
        const terrainTypeKey = game.board[gridY]?.[gridX] || 'PLAINS';
...
function updateCombat(game, deltaTime) {
    ...
    const allBuildings = Object.values(game.buildings);
    ...
    allUnits.forEach(u => {
        ...
        let target = u.targetId ? (game.units[u.targetId] || game.buildings[u.targetId]) : null;

        if (!target || deadEntities.units.has(target.id) || deadEntities.buildings.has(target.id)) {
            u.targetId = null;
            target = findClosestEnemy(u, allUnits, allBuildings, deadEntities);
            if (target) u.targetId = target.id;
        }

        if (target) {
            const targetSize = target.type ? (target.type === 'VEZ' ? 2 : 3) : 1; // unit size is 1
            const targetX = target.x + (targetSize > 1 ? targetSize/2 : 0);
            const targetY = target.y + (targetSize > 1 ? targetSize/2 : 0);
            ...
            if (distSq <= u_conf.range**2) {
                ...
                if (u.attackCooldown <= 0) {
                    ...
                    if(target.buildProgress === undefined){ // It's a unit
                        const modifier = GAME_CONFIG.RPS_MODIFIERS[u.type]?.[target.type] || 1;
                        damage *= modifier;
                    }
                    target.hp -= damage;
                    
                    game.dirtyData.events.push({ type: 'ATTACK_EFFECT', from: {x: u.x, y: u.y}, to: {x: targetX, y: targetY}, unitType: u.type });
                    target.buildProgress === undefined ? game.dirtyData.units.add(target.id) : game.dirtyData.buildings.add(target.id);
                    
                    u.attackCooldown = 1 / u_conf.attack_speed;

                    if (target.hp <= 0) {
                        target.buildProgress === undefined ? deadEntities.units.add(target.id) : deadEntities.buildings.add(target.id);
                    }
                }
            } else { 
        ...
        if (target) {
            ...
            if (distSq <= b_conf.range**2) {
                 if (b.attackCooldown <= 0) {
                    ...
                    game.dirtyData.events.push({ type: 'ATTACK_EFFECT', from: {x: b_center_x, y: b_center_y}, to: {x: target.x, y: target.y}, unitType: b.type });
    ...
    if (deadEntities.units.size > 0) {
        ...
        game.dirtyData.events.push({ type: 'UNITS_DIED', ids: Array.from(deadEntities.units) });
    }
     if (deadEntities.buildings.size > 0) {
        deadEntities.buildings.forEach(deadId => {
            const deadBuilding = game.buildings[deadId];
            if (deadBuilding) {
                // TODO: Check if it's a base and end game
                delete game.buildings[deadId];
            }
        });
        // We can add a BUILDING_DIED event if needed
    }
}

function findClosestEnemy(attacker, allUnits, allBuildings, deadEntities) {
    let closestEnemy = null;
    let min_dist_sq = (GAME_CONFIG.UNITS[attacker.type]?.vision || GAME_CONFIG.BUILDINGS[attacker.type]?.vision)**2;
    const attacker_center_x = attacker.x + (attacker.buildProgress !== undefined ? (attacker.type === 'VEZ' ? 1 : 1.5) : 0);
    const attacker_center_y = attacker.y + (attacker.buildProgress !== undefined ? (attacker.type === 'VEZ' ? 1 : 1.5) : 0);
    ...
    // Attacking buildings is only for units for now
    if(attacker.buildProgress === undefined){
...
function handlePlayerAction(socket, action) {
    ...
    switch (action.type) {
        ...
        case 'ATTACK_TARGET': {
            ...
            const target = game.units[targetId] || game.buildings[targetId];
            if (!target) return;
            unitIds.forEach(id => {
                 const unit = pData.units[id];
        ...
        case 'BUILD_STRUCTURE': {
            ...
            if (builder?.can_build && config && canAfford(pData, config.cost)) {
                const terrainType = game.board[position.y]?.[position.x];
                ...
                const b_size = (structureType === 'VEZ' ? 2 : 3);
                const newBuilding = {
                    id: createId(), ownerId: pData.id, type: structureType, x: position.x, y: position.y, hp: 1, maxHp: config.hp,
                    buildProgress: 0, buildTime: config.build_time, trainingQueue: [], rallyPoint: {x: position.x + b_size/2, y: position.y + b_size + 1},
                    attackCooldown: 0, targetUnitId: null // for towers
                };
...
function initializeGame(game) {
    ...
    game.players.forEach((player, index) => {
        ...
        createUnit(game, player, 'STAVITEL', {x: pos.x + 4, y: pos.y + 1});
        createUnit(game, player, 'PECHOTA', {x: pos.x - 1, y: pos.y + 1});
        createUnit(game, player, 'PECHOTA', {x: pos.x + 1.5, y: pos.y + 4});
...
// --- END OF FILE server.js (Absolutní finální verze) ---